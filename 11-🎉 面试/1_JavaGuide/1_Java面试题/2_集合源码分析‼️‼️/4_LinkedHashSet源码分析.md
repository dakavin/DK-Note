---
文章标题: "[[4_LinkedHashSet源码分析]]" 
文章作者: Dakkk
文章概要: |
  本文深入剖析Java `LinkedHashMap` 的底层实现原理，阐述其如何通过双向链表实现插入顺序/访问顺序迭代和LRU缓存机制。文章通过源码分析 `Node` 设计及关键方法的内部运作，并对比 `HashMap` 性能。
文章标签:
- "LinkedHashMap"
- "Java集合框架"
- "数据结构"
...
- "[[3_链表（全）]]"
文章分类: "🎉 面试"
文章路径: "11-🎉 面试/1_JavaGuide/1_Java面试题/2_集合源码分析‼️‼️/4_LinkedHashSet源码分析.md"
文章难度: 中级 🌳
目前阶段: ✅ 已完成
重要性: ⭐⭐⭐⭐⭐ 精通必备
...
前置文章：[8.4 LinkedHashMap源码剖析](../../../../../2_笔记/1_Java基础/14_数据结构与集合源码/14_数据结构与集合源码.md#8.4%20LinkedHashMap源码剖析)

总结：
- LinkedHashMap在HashMap的基础上，为每一个元素都维护了一个双向链表；
- 所以其可以按照插入顺序获取元素
...
1. 支持`遍历时会按照插入顺序有序进行迭代`。
2. 支持`按照元素访问顺序排序,适用于封装 LRU 缓存工具`。
