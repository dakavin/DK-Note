...
文章作者: Dakkk
文章概要: |
  本文系统介绍了Java多线程编程，涵盖程序、进程、线程等核心概念，以及两种线程创建方式。深入讲解了线程生命周期、并发与并行。重点阐述了线程安全问题及其同步机制（synchronized、Lock），线程间通信（wait/notify）与死锁。最后引入了Callable接口和线程池等高级特性。
文章标签:
- "Java多线程"
- "进程与线程"
- "线程生命周期"
...
相关文章: 暂无 🤷
文章分类: "💻 编程语言"
文章路径: "02-💻 编程语言/01-☕ Java技术栈/02-📚 基础语法/1_Java基础/9_多线程.md"
文章难度: 中级 🌳
目前阶段: ✅ 已完成
重要性: ⭐⭐⭐⭐⭐ 精通必备
创建时间: 2024-08-11 18:15:12
修改时间: 2025-05-26 07:14:45
---

## 1 本章专题与脉络

![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/ed0e749ae997b5497d281441feade3ef.png)
...
* **线程（thread）**：进程可进一步细化为线程，是程序内部的`一条执行路径`。一个进程中至少有一个线程。
  ...
  - 一个进程同一时间若`并行`执行多个线程，就是支持多线程的。
    ![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/e908f80ac21081618870f05d482db6f2.png)
  ...
  ![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/e5f15a560a7e95304d569cb6912394bf.png)
  > 注意：
  >
  > 不同的进程之间是不共享内存的。
  >
...
- **抢占式调度**
  ...
  让`优先级高`的线程以`较大的概率`优先使用 CPU。如果线程的优先级相同，那么会随机选择一个(线程随机性)，Java使用的为抢占式调度。
	![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/d609584e073b2e4ec76d3bccb89420ce.bmp)
...
* **并行（parallel）**：指两个或多个事件在`同一时刻`发生（同时发生）。指在同一时刻，有`多条指令`在`多个CPU`上`同时`执行。比如：多个人同时做不同的事。
  ![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/21b39c9b17297f4663f173799ca03ce4.png)

* **并发（concurrency）**：指两个或多个事件在`同一个时间段内`发生。即在一段时间内，有`多条指令`在`单个CPU`上`快速轮换、交替`执行，使得在宏观上具有多个进程同时执行的效果。
  ![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/7a14a82825accb0020ff9fe8e2554e22.png)

在操作系统中，启动了多个程序，`并发`指的是在==一段时间内宏观上有多个程序同时运行==，这在单核 CPU 系统中，每一时刻只能有一个程序执行，即微观上这些程序是分时的交替运行，只不过是给人的感觉是同时运行，那是因为分时交替运行的时间是非常短的。

而在多核 CPU 系统中，则这些可以`并发`执行的程序便可以分配到多个CPU上，实现多任务并行执行，即==利用每个处理器来处理一个可以并发执行的程序==，这样多个程序便可以同时执行。目前电脑市场上说的多核 CPU，便是多核处理器，核越多，`并行`处理的程序越多，能大大的提高电脑运行的效率。
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/6724cdf9b7a114dfb6059dd8c6591190.png)
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/c0605ded5e85ca3555f50a52f60f799b.png)
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/b6c8edf5aae671dbd45c2489ba186b36.png)
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/9973c5d83f16717f11e103fb6c2c75ab.png)
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/cbdd00e031dcf79d0e70bd0c165cfad7.png)


或

![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/2f91fb9db5f2e52faa85d255066bfa7e.png)
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/e76e5b122f3cfa6dafe85b28d12d1cbf.png)
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/f02867728ab9901ab9a3a6d8dbfd6473.png)
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/3b343e42183b24e8cc57aa26d4bf59b1.png)
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/cd5052f1b617c07a05e813848c45dcb2.png)
...
### 8.4 调用wait和notify需注意的细节

1. wait方法与notify方法必须要由`同一个锁对象调用`。因为：对应的锁对象可以通过notify唤醒使用同一个锁对象调用的wait方法后的线程。
2. wait方法与notify方法是属于Object类的方法的。因为：锁对象可以是任意对象，而任意对象的所属类都是继承了Object类的。
3. wait方法与notify方法必须要在`同步代码块`或者是`同步方法`中使用。因为：必须要`通过锁对象`调用这2个方法。否则会报java.lang.IllegalMonitorStateException异常。
...
![|380](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/04/4221095f93c1d63be7367c3e2009e715.png)
