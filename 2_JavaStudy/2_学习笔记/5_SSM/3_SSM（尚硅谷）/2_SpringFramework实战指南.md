...
- 关于AOP导包的问题（aspectjweaver和spring-aspects的区别）
	1. `aspectjweaver`是AspectJ的运行时库，它提供了实现切面编程的核心功能。AspectJ是一个功能强大的切面编程框架，它允许在Java应用程序中定义和应用切面。`aspectjweaver`库包含了AspectJ的核心功能和运行时工具，可以在运行时织入和执行切面代码。使用AspectJ，你可以定义切面、切点和通知，来实现横切关注点的处理。
    
	1. `spring-aspects`是Spring Framework提供的一个模块，它使用了AspectJ和`aspectjweaver`库，并提供了与Spring框架集成的切面编程功能。`spring-aspects`模块为Spring应用程序提供了一些预定义的切面，例如事务管理的切面(`@Transactional`)和异步执行的切面(`@Async`)等。通过使用`spring-aspects`，你可以将这些预定义的切面直接应用到你的Spring应用程序中，无需手动编写AspectJ切面。

- 简而言之，`aspectjweaver`提供了AspectJ切面编程框架的核心功能和运行时支持，可以独立使用。而`spring-aspects`是Spring Framework的一个模块，它集成了AspectJ和`aspectjweaver`，提供了一些与Spring框架集成的切面功能。

- 如果你只需要使用AspectJ的核心功能，独立于Spring Framework，可以直接使用`aspectjweaver`。如果你正在开发基于Spring Framework的应用程序，并且需要使用Spring提供的切面功能，那么可以选择使用`spring-aspects`模块。
...
- 注意：`@Autowired声明的是接口，而不是实现类`
- 因为：AOP代理模式，使用jdk进行接口的代理，所以具体的实现类，和IOC中的代理类会发生冲突！！！
...
4.  切点统一管理
    ...
    这样的话，在其他通知类，需要引用切入点的时候，需要写出切入点的全类名
    ```java
    @Component
