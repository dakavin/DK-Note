...
// ===== 配置区域 =====
// 修改这里来切换查看不同月份的记录
const BASE_PATH = "12-📅 日常记录/01-📆 每日计划/01-🍃 2025年/05月🌦️";
const REFERENCE_DATE = dv.date("2025-05-01"); // 可以改为具体日期，如 dv.date("2025-01-15")
// 计算本周范围
...
TABLE WITHOUT ID 
    ...
    choice(split(活动, "-")[4] AND number(split(活动, "-")[4]), 
           ...
           "未知") as "时长",
    split(活动, "-")[5] as "备注"
FROM "12-📅 日常记录/01-📆 每日计划/01-🍃 2025年/05月🌦️"
WHERE contains(file.name, "Daily Plan") 
    AND date(split(file.name, " - ")[1]) >= (date("2025-05-01") - dur(string(date("2025-05-01").weekday) + " days"))
    AND date(split(file.name, " - ")[1]) <= (date("2025-05-01") + dur(string(6 - date("2025-05-01").weekday) + " days"))
FLATTEN 活动
WHERE 活动 
    AND contains(活动, "-") 
...
WHERE contains(file.name, "Daily Plan") 
  AND 活动
  AND date(split(file.name, " - ")[1]) >= (date("2025-05-01") - dur("13 days"))
SORT file.name DESC
```
...
FROM "12-📅 日常记录/01-📆 每日计划/01-🍃 2025年/05月🌦️"
WHERE contains(file.name, "Daily Plan") 
  AND date(split(file.name, " - ")[1]).year = date("2025-05-01").year
  AND date(split(file.name, " - ")[1]).month = date("2025-05-01").month
FLATTEN 活动
WHERE 活动 
...
```dataview
LIST WITHOUT ID
"📊 **本月活动统计 (" + date("2025-05-01").year + "-" + string(padleft(string(date("2025-05-01").month), 2, "0")) + ")**" + "  
- 总活动时长：" + choice(
    sum(map(rows, (r) => choice(split(r.活动, "-")[4] AND number(split(r.活动, "-")[4]), number(split(r.活动, "-")[4]), 0))) > 0,
    choice(
      sum(map(rows, (r) => choice(split(r.活动, "-")[4] AND number(split(r.活动, "-")[4]), number(split(r.活动, "-")[4]), 0))) >= 60,
...
```dataview
LIST WITHOUT ID
"📚 **本月学习统计 (" + date("2025-05-01").year + "-" + string(padleft(string(date("2025-05-01").month), 2, "0")) + ")**" + "  
- 总学习时长：" + choice(
    sum(map(rows, (r) => choice(split(r.活动, "-")[4] AND number(split(r.活动, "-")[4]), number(split(r.活动, "-")[4]), 0))) > 0,
    choice(
      sum(map(rows, (r) => choice(split(r.活动, "-")[4] AND number(split(r.活动, "-")[4]), number(split(r.活动, "-")[4]), 0))) >= 60,
