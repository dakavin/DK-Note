上小节中，我们已经给后端服务添加了获取标签-文章列表分页接口。本小节中，我们就来将标签文章页开发完成，包括请求接口动态渲染数据，分页，以及路由跳转。

![](https://img.quanxiaoha.com/quanxiaoha/169899739801685)

## 1. 封装请求标签文章分页方法

首先，编辑 `/api/frontend/tag.js` 文件，封装一个请求标签文章分页接口的方法，代码如下：
...
从本小节开头贴出的原型图可以看到，基本上和之前实现的分类文章列表页差不多。在 `/pages/frontend/` 文件夹下，将 `category-article-list.vue` 文件复制一份，命名为 `tag-article-list.vue` ，代码大体上都不需要动，仅需要将接口调用，变量名，方法名修改一下，代码如下：
...
> **TIP** : 这个页面中的标签图标，小哈并没有使用的 `flowbit` 官网提供的图标，想要来点彩色的图标，更好看些，所以从阿里图标网站：[https://www.iconfont.cn](https://www.iconfont.cn/) ，查询关键词 ：_标签_， 选的一个个人感觉还不错的，复制其 `svg`代码：
> 
> ![](https://img.quanxiaoha.com/quanxiaoha/169899359960263)
> 
> 既然这个页面中，标签图标换成了一个彩色图标，顺便也把之前分类文章列表中的文件夹图标也换一个，同样的步骤，搜素关键词：_文件夹_，找到一个和博客主色调接近的，小哈这里选择的下面这个：
> 
> ![](https://img.quanxiaoha.com/quanxiaoha/169899872368791)

`svg` 代码如下，编辑 `category-article-list.vue` 页面中文件夹图标相关代码，如下：
...
注意：复制的svg代码，其中class需要改为如下 `class="icon w-5 h-5 mr-2"`

效果图：

![image.png|500](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/05/e023197a3c51c42849fbc956789003cf.png)

## 3. 路由注册

页面写好后，我们编辑 `/router/index.js` 文件，注册其路由：
...
接下来，编辑 `/layouts/frontend/components/TagListCard.vue` 组件，为每个标签添加 `@click` 点击事件，跳转标签文章列表页路由，代码如下：
...
![](https://img.quanxiaoha.com/quanxiaoha/169899354809679)

## 5. 小作业

本小节的最后，给小伙伴们布置点作业，尝试自己动手：

- **第一个作业**：给首页文章卡片中的标签，以及标签列表页，也添加上路由跳转；

给标签列表页（`src/pages/frontend/tag-list.vue`）添加路由跳转如下：
![image.png|500](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/05/7219d3469f627671224d24c45b3ae9e8.png)

给首页文章卡片中的标签（`src/pages/frontend/index.vue`）添加路由跳转如下：
![image.png|500](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/05/3ba35cde0b63cb332d78c3c1f83f5ab8.png)

- **第二个作业**：为右侧栏的卡片，如分类卡片、标签卡片的标题前面，添加个 `svg` 图标，让 `UI` 更帅些，图标就用各自列表页的图标，效果图如下：

标签卡片和分类卡片代码如下：
![image.png|500](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/05/d37a9555dd92bc80cbdb807fa5a60f89.png)
