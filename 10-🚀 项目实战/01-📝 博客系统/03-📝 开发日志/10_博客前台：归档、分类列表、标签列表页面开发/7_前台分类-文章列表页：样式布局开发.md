---
文章标题: "[[7_前台分类-文章列表页：样式布局开发]]" 
文章作者: Dakkk
文章概要: |
  文章讲解了使用Vue.js和Vue Router构建博客分类文章列表页前端样式。涵盖新页面创建、路由配置、带参跳转、列表与分页布局，以及动态显示分类名称和处理无文章空状态提示，体现组件化和响应式设计。
文章标签:
- "Vue.js"
- "Vue Router"
- "前端开发"
...
- "[[6_首页样式布局设计（1）]]"
文章分类: "🚀 项目实战"
文章路径: "10-🚀 项目实战/01-📝 博客系统/03-📝 开发日志/10_博客前台：归档、分类列表、标签列表页面开发/7_前台分类-文章列表页：样式布局开发.md"
文章难度: 初级 💧
目前阶段: ✅ 已完成
重要性: ⭐⭐⭐⭐ 核心能力
...
本小节中，我们将完成分类-文章列表页的样式开发工作。即当用户点击某个分类时，需要跳转分类-文章列表页，只展示该分类下的文章，如下图所示：

![](https://img.quanxiaoha.com/quanxiaoha/169864347721596)

另外，当该分类下没有文章时，则提示用户 _此分类下还未发布文章哟_ 的提示，如下图所示：

![](https://img.quanxiaoha.com/quanxiaoha/169864337655712)

## 1 基础布局

我们在 `/pages/frontend` 文件夹下，新创建 `category-article-list.vue` 分类文章列表页 ，先将基础布局弄好，代码如下：

![image.png|500](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/05/cc2a8123d7a734da313b9f61f02eb485.png)
...
和之前一样，依然是左右布局，直接从首页中将相关代码复制过来就行。

## 2 注册路由

页面写好后，编辑 `/router/index.js` 文件，添加分类文章页的路由，代码如下：
...
为了看到页面效果，接下来，编辑 `/layouts/frontend/components/CategoryListCard.vue` 分类卡片组件，为分类 `<a>` 标签添加点击事件，并实现路由跳转，代码如下：
...
与前面小节中的路由跳转不同，这次我们跳转的同时，还需要携带两个参数，分别是分类 `ID` 以及分类名称， 分类 `ID` 是为了跳转过去后，请求分页接口需要；而分类名称则是页面需要展示 。

![image.png|500](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/05/449a723a3ace44b16cf7ccc484865f6a.png)
## 4 填充文章列表样式布局

目前主内容部分还是空缺的，这块的样式布局代码和归档页差不多，直接复制过来，只需要额外给头部的标题前面添加一个文件夹图标，意味分类列表即可，代码如下：

> `svg` 图标可以从 `Flowbit` 官网搜索，并复制：[https://flowbite.com/icons/](https://flowbite.com/icons/) 。
...
然后是下方的分页部分，依然是直接从首页复制过来，点击事件、逻辑判断啥的先删除掉，代码如下：
...
![image.png|500](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/05/114e424ed68b1645329ceaf6ee70b414.png)

## 6 动态显示分类名称

前面跳转路由时，我们已经将分类名称携带过来了，现在只需要渲染即可：
...
上述代码中，我们通过 `route` 来获取路由跳转携带过来的分类名称，以及 `ID`。主要注意的是，在 `Vue Router` 中，当我们使用 `router.push()` 导航到同一个路由时，默认情况下不触发路由更新的，也就是说，如果直接通过 `route.query.xx` 来获取，那么拿到的还是第一次刷新的值，为了能够实时获取当前路由携带的参数，可以通过侦听属性 `watch` 来对 `route` 进行监听，就能拿到最新的值，并设置到对应响应式变量中。

![image.png|500](https://my-obsidian-image.oss-cn-guangzhou.aliyuncs.com/2024/05/daa55e8660976c1bc69c028d78a20ac0.png)

## 7 未发布文章提示: 样式布局

文章列表样式布局弄完后，还需要设计一下当该分类下没有文章的提示部分，通常结构都是一张图片 + 提示消息，图片我们可以从阿里的图标网站来找一张插画：[https://www.iconfont.cn/](https://www.iconfont.cn/) ，下面是小哈找的比较符合语境的插画，直接点击 _复制 SVG 代码_：

![](https://img.quanxiaoha.com/quanxiaoha/169864678916390)

编辑 `category-article-list.vue` 页面，在列表下方再添加一个 `<div>` 容器，并将 `svg` 代码粘贴进去，然后，修改一下大小，将 `height` 高度改为 300, 代码如下：
...
为了能够查看效果，我们还定义了一个响应式变量 `articles` , 默认长度为 0，并在文章列表，以及分页上添加判断，若长度大于 0 则显示，否则则显示未发布文章提示：
...
回到浏览器，即可看到未发布文章的提示效果了：

![](https://img.quanxiaoha.com/quanxiaoha/169864337655712)

到这里，分类文章列表页的样式布局就设计完成了。